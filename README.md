...
<!-- Add a real Settings screenshot before release -->
...
For providers using multiple keys, the quota panel currently queries only the first configured key and labels this in the UI.
...
- The quota panel currently shows quota for only the first key of a multi-key provider.
