 ...
 */
export function createExaAdvancedTool(runtime: AdvancedSearchRuntime): ToolDefinition {
  return defineTool({
    name: "web_search_exa",
    description:
      "Advanced neural search powered by Exa. Use when you need deep search, " +
    ...
    parameters: {
      ...
      type: {
        ...
        description:
          "Search mode. 'auto' (default) lets Exa choose. 'instant' (<250ms, low cost). " +
      ...
      category: {
        ...
        description:
          "Target content category. Narrows results to specific data types. " +
    ...
    output: {
      ...
      render(_args, value) {
        return [
          {
            type: "text",
            text: formatOutcomeText(value as unknown as AdvancedSearchOutcome),
          },
        ];
      },
    ...
    async execute(args, exec) {
      ...
      if (active !== "exa") {
        return runtime.buildStaleProviderTransition("exa", active) as any;
      }
 ...
 */
export function formatOutcomeText(outcome: AdvancedSearchOutcome): string {
  switch (outcome.kind) {
    case "success": {
      ...
      outcome.sources.forEach((s: AdvancedSearchSource, i: number) => {
        ...
        if (s.score !== undefined) lines.push(`Score: ${s.score.toFixed(3)}`);
    ...
    case "provider_transition": {
      ...
      const nextActionDesc =
        t.nextTool && t.nextTool !== "web_search"
          ? `Call tool '${t.nextTool}' with provider-native arguments.`
          : "Use basic 'web_search' tool.";
      return (
        `[Search Provider Transition]\n` +
    ...
    case "configuration_error": {
      const f = outcome.failure;
      return (
        `[Exa Configuration Error: ${f.code}]\n` +
        `${f.message}\n` +
