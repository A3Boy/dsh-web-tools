...
const NM = "D:/web/dsh-plugins/dsh-web-tools/node_modules/@deepseek-ai";
 ...
 * the known candidates at load time instead of hardcoding one path that breaks
...
const DSH_NM =
  DSH_NM_CANDIDATES.find((base) => existsSync(`${base}/dsh-tool-web/lib/index.js`)) ??
  DSH_NM_CANDIDATES[0];
...
const toolWebPlugin = { name: "tool-web", inject: toolWebInject, apply: applyToolWeb };
...
async function buildRoot() {
  ...
  await root.plugin(toolWebPlugin, HOST_TOOL_WEB_CONFIG);
  return root;
}

test("preset fetch:false does NOT tombstone the global web_fetch", async () => {
  ...
  await standing.ctx.plugin(toolWebPlugin, PRESET_TOOL_WEB_CONFIG);
...
test("a preset without tool-web (minimal) inherits the global web tools", async () => {
  const root = await buildRoot();
