...
function CredentialDisclosure(props) {
    ...
    const summaryColor = keys.length === 0
        ...
        : allHealthy
            ? text.secondary
            : stateColor.danger;
    return (_jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px" }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontWeight: 600, fontSize: 13, color: text.primary }, children: t("credentials") }), _jsx("span", { style: { fontSize: 12, fontWeight: allHealthy ? 400 : 600, color: summaryColor }, children: summaryText })] }), p.keyWritable && (_jsx("div", { style: { marginTop: 2, display: "flex", flexDirection: "column", gap: 8 }, children: _jsx(CredentialList, { t: t, p: p, onChanged: onChanged, onError: onError, onTest: onTest, busy: busy, testResult: testResult }) }))] }));
}
...
function CredentialList(props) {
    ...
    return (_jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: [keys.map((k) => (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, fontSize: 13, minHeight: 28 }, children: [_jsx("span", { style: { fontFamily: "var(--ds-font-family-code, ui-monospace, Menlo, Consolas, monospace)", color: text.primary, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: k.hint }), _jsxs("span", { style: { display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, color: k.healthy ? stateColor.success : stateColor.danger, whiteSpace: "nowrap" }, children: [_jsx("span", { style: { width: 6, height: 6, borderRadius: "50%", background: k.healthy ? stateColor.success : stateColor.danger } }), k.healthy ? t("keyReady") : t("keyAuthError")] }), _jsx("div", { style: { marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6 }, children: confirmKeyId === k.id ? (_jsxs("div", { style: { display: "inline-flex", alignItems: "center", gap: 4, background: surface.layer2, padding: "2px 6px", borderRadius: 6, border: `1px solid ${surface.border}` }, children: [_jsx("span", { style: { fontSize: 11, color: text.secondary }, children: "\u786E\u8BA4\u5220\u9664?" }), _jsx(Button, { size: "sm", variant: "ghost", onClick: () => void removeKey(k.id), disabled: busyKey === k.id, style: { color: stateColor.danger, padding: "0 4px", height: 20 }, children: "\u5220\u9664" }), _jsx(Button, { size: "sm", variant: "ghost", onClick: () => setConfirmKeyId(null), style: { padding: "0 4px", height: 20 }, children: "\u53D6\u6D88" })] })) : (_jsx(Button, { size: "sm", variant: "ghost", icon: _jsx(IconTrashOutline16, { size: 14 }), onClick: () => setConfirmKeyId(k.id), disabled: busyKey === k.id, "aria-label": t("removeKey") })) })] }, k.id))), adding ? (_jsxs("div", { style: { display: "flex", gap: 6, alignItems: "center", marginTop: 4 }, children: [_jsx("input", { autoFocus: true, type: "password", value: draft, onChange: (e) => setDraft(e.target.value), onKeyDown: (e) => { if (e.key === "Enter")
...
export function ProviderModal(props) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: `
      ...
      ` }), _jsx(Modal, { open: true, onClose: onClose, title: p.label, closeLabel: t("close"), description: t(`capability.${p.name}`) || "", className: "wt-modal-dialog", contentClassName: "wt-modal-content", children: _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 14, paddingBottom: 8 }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 0 2px" }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10 }, children: [brand && (_jsx("img", { src: brand.icon, alt: p.label, width: 28, height: 28, style: { borderRadius: 6, flexShrink: 0, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" } })), _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 1 }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 6 }, children: [_jsx("span", { style: { fontWeight: 600, fontSize: 15, color: text.primary }, children: p.label }), showPreferred && _jsx("span", { style: { fontSize: 11, padding: "1px 6px", borderRadius: 4, background: "color-mix(in srgb, var(--dsw-alias-brand-primary) 12%, transparent)", color: "var(--dsw-alias-brand-primary)", fontWeight: 500 }, children: t("preferredProviderLabel") })] }), _jsx("span", { style: { fontSize: 12, color: text.tertiary }, children: t(`capability.${p.name}`) || "" })] })] }), _jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10 }, children: [status !== "ready" && (_jsxs("span", { style: { display: "inline-flex", alignItems: "center", gap: 5 }, children: [statusState === "hollow" ? (_jsx("span", { "aria-hidden": true, style: { width: 8, height: 8, borderRadius: "50%", border: `1.5px solid ${text.tertiary}`, flex: "none", boxSizing: "border-box" } })) : (_jsx(StateDot, { state: statusState, size: 8 })), _jsx("span", { style: { color: statusColor, fontWeight: 500, fontSize: 12 }, children: statusText })] })), _jsx(Switch, { checked: p.enabled, onChange: onToggle, label: p.enabled ? t("enabledLabel") : t("disabledLabel") })] })] }), _jsxs(SettingsGroup, { children: [!selfHosted && (_jsx(CredentialDisclosure, { t: t, p: p, onChanged: onConfigChanged, onError: setLocalError, onTest: onTest, busy: busy, testResult: testResult })), (selfHosted || p.baseUrl !== undefined) && (_jsx(ConnectionSettingsDisclosure, { t: t, p: p, draftBaseUrl: draftBaseUrl, setDraftBaseUrl: setDraftBaseUrl, onBaseUrl: onBaseUrl }))] }), quota && _jsx(QuotaCard, { quota: quota, t: t, onRefresh: onRefreshQuota }), p.options && p.name !== "searxng" && (_jsx(SettingsGroup, { title: t("prefsTitle"), children: _jsx("div", { style: { padding: "12px 14px" }, children: _jsx(ProviderPreferencesSection, { t: t, p: p, onConfigChanged: onConfigChanged }) }) })), _jsx(SettingsGroup, { children: _jsx(DeveloperOptions, { t: t, p: p, onConfigChanged: onConfigChanged }) }), localError && _jsx("div", { style: { color: stateColor.danger, fontSize: 12 }, children: localError })] }) })] }));
}
