...
function CredentialDisclosure(props) {
    ...
    return (_jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: [_jsxs("div", { role: "button", tabIndex: 0, "aria-expanded": open, onClick: () => canOpen && setOpen(!open), onKeyDown: (e) => { if ((e.key === "Enter" || e.key === " ") && canOpen) {
                    e.preventDefault();
                    setOpen(!open);
                } }, style: { display: "flex", alignItems: "center", gap: 8, cursor: canOpen ? "pointer" : "default", fontSize: 12, color: text.secondary, outline: "none" }, children: [keys.length > 0 && (_jsx("span", { style: { transform: open ? "rotate(90deg)" : "none", transition: "transform .15s ease", flex: "none", color: text.tertiary, display: "inline-flex" }, children: _jsx(IconChevronRightOutline14, { size: 12 }) })), _jsx("span", { children: summary }), keys.length === 0 && canOpen && (_jsx(Button, { size: "sm", variant: "ghost", onClick: () => setOpen(true), style: { marginLeft: "auto", fontSize: 11 }, children: t("addKey") })), keys.length > 0 && (_jsx(Button, { size: "sm", variant: "ghost", onClick: () => setOpen(!open), style: { marginLeft: "auto", fontSize: 11 }, children: open ? t("collapse") : t("manage") }))] }), open && p.keyWritable && (_jsx(CredentialList, { t: t, p: p, onChanged: onChanged, onError: onError }))] }));
}
