...
function ProviderCard(props) {
    ...
    return (_jsxs("div", { onClick: onClick, className: "wt-provider-row", role: "button", tabIndex: 0, onKeyDown: (e) => { if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            onClick();
        } }, style: {
            display: "flex",
        ...
        }, onMouseEnter: (e) => { e.currentTarget.style.background = surface.hover; }, onMouseLeave: (e) => { e.currentTarget.style.background = surface.layer1; }, children: [PROVIDER_BRAND[p.name] && (_jsx("img", { src: PROVIDER_BRAND[p.name].icon, alt: "", width: 24, height: 24, style: { borderRadius: 6, flex: "none" } })), _jsx("span", { style: { fontWeight: 500, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: "none" }, children: p.label }), showPreferred && (_jsx("span", { style: { color: accentText(), fontSize: 11, fontWeight: 600, border: "1px solid currentColor", borderRadius: 4, padding: "0 6px", whiteSpace: "nowrap" }, children: t("preferredProviderLabel") })), _jsxs("span", { style: { marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6, flex: "none" }, children: [dotState === "hollow" ? (_jsx("span", { "aria-hidden": true, style: { width: 8, height: 8, borderRadius: "50%", border: `1.5px solid ${text.tertiary}`, flex: "none", boxSizing: "border-box" } })) : (_jsx(StateDot, { state: dotState, size: 8 })), _jsx("span", { style: { color: statusColor, fontSize: 12, whiteSpace: "nowrap" }, children: statusText })] }), _jsx(IconChevronRightOutline14, { size: 14 })] }));
}
function accentText() {
...
export function WebToolsSection(props) {
    ...
    const [uiPref, setUiPref] = useState("auto");
    ...
    useEffect(() => {
        if (config)
            setUiPref(config.uiLanguage ?? "auto");
    }, [config]);
    ...
    return (_jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 12, maxWidth: 720, padding: "4px 0 24px" }, children: [_jsx("style", { children: `
      ...
      ` }), _jsxs("div", { style: { display: "flex", alignItems: "flex-start", gap: 12 }, children: [_jsxs("div", { style: { flex: 1, minWidth: 0 }, children: [_jsx("h2", { style: { margin: 0, fontSize: 16, fontWeight: 500, lineHeight: "24px", color: text.primary }, children: t("title") }), _jsx("p", { style: { margin: "2px 0 0", fontSize: 14, lineHeight: "22px", color: text.tertiary }, children: t("tagline") })] }), _jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, paddingTop: 2, flex: "none", flexWrap: "wrap", justifyContent: "flex-end" }, children: [_jsx(Switch, { checked: config.enabled, onChange: setEnabled, label: config.enabled ? t("enabledLabel") : t("disabledLabel") }), saving && _jsx("span", { style: { color: text.tertiary, fontSize: 12 }, children: t("saving") }), saved && !saving && _jsx("span", { style: { color: stateColor.success, fontSize: 12 }, children: t("saved") })] })] }), error && _jsx("div", { style: { color: stateColor.danger, fontSize: 13 }, children: error }), config.proxy?.configured === true && config.proxy?.degraded === true && (_jsxs("div", { style: {
                    ...
                    color: text.secondary,
                }, children: [_jsx("strong", { style: { color: stateColor.warning, fontSize: 13 }, children: t("proxyDegradedTitle") }), _jsx("span", { children: t("proxyDegradedBody") })] })), _jsx("section", { style: { display: "flex", flexDirection: "column", gap: 8 }, children: _jsxs("div", { style: {
                        display: "flex",
                        alignItems: "center",
                        ...
                        background: surface.layer1,
                        border: `1px solid ${surface.border}`,
                    }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, minWidth: 0, overflow: "hidden" }, children: [_jsx("span", { style: { fontSize: 13, fontWeight: 500, color: text.primary, whiteSpace: "nowrap" }, children: t("routingLabel") }), _jsx("span", { style: { color: text.tertiary }, children: "\u00B7" }), _jsx("span", { style: { fontSize: 13, fontWeight: 500, color: text.primary, whiteSpace: "nowrap" }, children: t(`routingPolicy.${config.searchRoutingPolicy ?? "ordered"}`) }), _jsx("span", { style: { color: text.tertiary }, children: "\u00B7" }), _jsx("span", { style: { fontSize: 12, color: text.tertiary, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: orderedProviders
                                        .map((name) => providerOf(name)?.label ?? name)
                                        .join((config.searchRoutingPolicy ?? "ordered") === "random" ? (t("save") === "保存" ? "、" : ", ") : " → ") })] }), _jsx(Button, { size: "sm", variant: "ghost", icon: _jsx(IconEditOutline16, { size: 14 }), onClick: () => setRoutingOpen(true), children: t("routingConfigure") })] }) }), _jsxs("section", { style: { display: "flex", flexDirection: "column", gap: 8 }, children: [_jsx("div", { style: { display: "flex", alignItems: "center" }, children: _jsx("h3", { style: { margin: 0, fontSize: 14, fontWeight: 500, color: text.primary }, children: t("providersLabel") }) }), _jsx("div", { style: { display: "flex", flexDirection: "column", gap: 8 }, children: renderedProviders.map((p) => {
                            const testResult = providerTestResults[p.name];
                            return (_jsx(ProviderCard, { t: t, p: p, quota: quotas?.[p.name], testResult: testResult, inOrder: orderedProviders.includes(p.name), showPreferred: showPreferredFor(p.name), onClick: () => setDetailFor(p.name) }, p.name));
                        }) })] }), _jsxs("details", { style: { fontSize: 13, borderTop: `1px solid ${surface.border}`, paddingTop: 12, marginTop: 4 }, children: [_jsx("summary", { style: { cursor: "pointer", color: text.secondary, fontSize: 13, fontWeight: 500, padding: "4px 0" }, children: t("diagnosticsAndMore") }), _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 16, padding: "12px 0 0" }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }, children: [_jsx("label", { style: { color: text.secondary, fontSize: 13 }, children: t("attemptTimeoutLabel") }), _jsx("input", { type: "number", min: 1000, max: 60000, step: 1000, value: config.providerAttemptTimeoutMs, onChange: (e) => setAttemptTimeout(Number(e.target.value)), style: {
                                            width: 90,
                                            padding: "4px 8px",
                                            borderRadius: 6,
                                            border: `1px solid ${surface.border}`,
                                            ...
                                            fontFamily: "inherit",
                                            fontSize: 13,
                                        } }), _jsxs("span", { style: { color: text.tertiary, fontSize: 12 }, children: [t("attemptTimeoutHint"), " (", Math.round(config.providerAttemptTimeoutMs / 1000), "s)"] })] }), _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 8 }, children: [_jsx("span", { style: { fontSize: 13, fontWeight: 500, color: text.primary }, children: t("testSearchTitle") }), _jsx(TestSearchBlock, { t: t, config: config, onError: (msg) => setError(msg) })] })] })] }), detailProvider && (_jsx(ProviderModal, { t: t, p: detailProvider, quota: quotas?.[detailProvider.name], testResult: providerTestResults[detailProvider.name], busy: !!busyProviders[detailProvider.name], showPreferred: showPreferredFor(detailProvider.name), inChain: orderedProviders.includes(detailProvider.name), onClose: () => { setDetailFor(null); setProviderTestResults((prev) => { const next = { ...prev }; delete next[detailProvider.name]; return next; }); }, onToggle: (enabled) => toggleProvider(detailProvider.name, enabled), onBaseUrl: (url) => setBaseUrl(detailProvider.name, url), onTest: () => testProvider(detailProvider.name), onRefreshQuota: () => void loadQuotas(true), onConfigChanged: () => {
