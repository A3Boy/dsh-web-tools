...
export function QuotaCard(props) {
    ...
    if (!quota || !quota.supported || quota.source === "dashboard") {
        return (_jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 10, padding: "12px 14px", borderRadius: 10, background: surface.layer1, border: `1px solid ${surface.border}` }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontSize: 12, fontWeight: 600, color: text.tertiary }, children: "\u4F7F\u7528\u7EDF\u8BA1\u4E0E\u989D\u5EA6" }), _jsx("button", { type: "button", onClick: () => void refresh(), disabled: refreshing, title: "\u5237\u65B0\u72B6\u6001", style: {
                                ...
                                display: "inline-flex",
                                alignItems: "center",
                            }, children: _jsx("span", { style: { display: "inline-flex", transform: refreshing ? "rotate(180deg)" : "none", transition: "transform .5s ease" }, children: _jsx(IconRefreshOutline16, { size: 13 }) }) })] }), _jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontSize: 13, color: text.secondary }, children: providerName === "parallel" || providerName === "exa" ? "按量计费 · 本地累计消耗 $0.00" : "控制台查询" }), dash && (_jsxs("a", { href: dash.url, target: "_blank", rel: "noreferrer", style: { color: "var(--dsw-alias-brand-primary)", textDecoration: "none", fontSize: 12, display: "inline-flex", alignItems: "center", gap: 2 }, children: [_jsx("span", { children: dash.label }), _jsx("span", { children: "\u2197" })] }))] })] }));
    }
    const { main, unit } = formatQuotaNumbers(quota);
    const fraction = quotaFraction(quota);
    const tier = quotaTier(fraction);
