...
export function QuotaCard(props) {
    ...
    if (!quota || !quota.supported || quota.source === "dashboard") {
        ...
        return (_jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 10, padding: "12px 14px", borderRadius: 10, background: surface.layer1, border: `1px solid ${surface.border}` }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontSize: 12, fontWeight: 600, color: text.tertiary }, children: t("quotaTitle") }), _jsx("button", { type: "button", onClick: () => void refresh(), disabled: refreshing, title: t("refreshQuota"), style: {
                            ...
                            }, children: _jsx("span", { style: { display: "inline-flex", transform: refreshing ? "rotate(180deg)" : "none", transition: "transform .5s ease" }, children: _jsx(IconRefreshOutline16, { size: 13 }) }) })] }), _jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontSize: 13, color: text.secondary }, children: isLocalMetered ? t("quotaSourceLocalEstimate") : t("quotaSourceDashboard") }), dash && (_jsxs("a", { href: dash.url, target: "_blank", rel: "noreferrer", style: { color: "var(--dsw-alias-brand-primary)", textDecoration: "none", fontSize: 12, display: "inline-flex", alignItems: "center", gap: 4 }, children: [_jsx("span", { children: t(dash.labelKey) }), _jsx(ExternalLinkIcon, { size: 12 })] }))] })] }));
    }
    const { main, unit } = formatQuotaNumbers(quota, t);
    const fraction = quotaFraction(quota);
    const tier = quotaTier(fraction);
