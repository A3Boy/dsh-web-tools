...
export function QuotaInline(props) {
    ...
    if (meteredNames.has(providerName ?? "")) {
        return (_jsx("span", { style: { fontSize: 12, fontWeight: 500, color: text.secondary, whiteSpace: "nowrap", flex: "none" }, children: t ? t("quotaMeteredPrefix") : "Pay-as-you-go" }));
    }
...
export function QuotaCard(props) {
    ...
    if (meteredNames.has(providerName ?? "")) {
        ...
        return (_jsxs("div", { style: cardShell(embedded), children: [_jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontSize: 12, fontWeight: 600, color: text.tertiary }, children: t("quotaTitle") }), _jsx(RefreshButton, { refreshing: refreshing, onRefresh: () => void refresh(), title: t("refreshQuota") })] }), _jsx("div", { style: { display: "flex", alignItems: "baseline", justifyContent: "space-between" }, children: _jsx("span", { style: { fontSize: 18, fontWeight: 600, color: text.primary }, children: t("quotaMeteredPrefix") }) }), _jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 11, color: text.tertiary, flexWrap: "wrap", gap: 6, paddingTop: 2 }, children: [_jsxs("span", { children: ["\u00B7 ", t(sourceLabel)] }), dash && (_jsxs("a", { href: dash.url, target: "_blank", rel: "noreferrer", style: { color: "var(--dsw-alias-brand-primary)", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 4 }, children: [_jsx("span", { children: t(dash.labelKey) }), _jsx(ExternalLinkIcon, { size: 12 })] }))] })] }));
    }
    // Fallback card when no quota snapshot or for dashboard-only providers
    if (!quota || !quota.supported || quota.source === "dashboard") {
        return (_jsxs("div", { style: cardShell(embedded), children: [_jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontSize: 12, fontWeight: 600, color: text.tertiary }, children: t("quotaTitle") }), _jsx(RefreshButton, { refreshing: refreshing, onRefresh: () => void refresh(), title: t("refreshQuota") })] }), _jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("span", { style: { fontSize: 13, color: text.secondary }, children: t("quotaSourceDashboard") }), dash && (_jsxs("a", { href: dash.url, target: "_blank", rel: "noreferrer", style: { color: "var(--dsw-alias-brand-primary)", textDecoration: "none", fontSize: 12, display: "inline-flex", alignItems: "center", gap: 4 }, children: [_jsx("span", { children: t(dash.labelKey) }), _jsx(ExternalLinkIcon, { size: 12 })] }))] })] }));
    }
    const { main, unit } = formatQuotaNumbers(quota, t);
    const fraction = quotaFraction(quota);
    const tier = quotaTier(fraction);
