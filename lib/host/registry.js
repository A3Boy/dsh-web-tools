...
import { buildPool, markUnhealthy, markUsed, selectIndex } from "./pool.js";
...
export function createSearchProvider(resolveConfig, resolveKeys, stats, adapterRegistry = PROVIDERS, poolStore) {
        ...
        async search(request, signal) {
            ...
            for (const providerName of chain) {
                ...
                while (usable.length > 0) {
                    ...
                    try {
                        const outcome = await runWithTimeout((signal) => adapter.search(request.query, maxResults, entry?.key ?? "", cfg.providerBaseUrls[providerName], signal), cfg.providerAttemptTimeoutMs, signal);
                        if (entry)
 ...
 */
export function createFetchProvider(resolveConfig, resolveKeys, adapterRegistry = PROVIDERS, poolStore) {
    const pools = poolStore ?? createPoolStore(resolveKeys);
    return {
        ...
        async fetch(request, signal) {
            ...
            for (const providerName of chain) {
                ...
                const entry = entries[index];
                try {
                    const { text } = await runWithTimeout((sig) => adapter.fetch(request.url, entry?.key ?? "", cfg.providerBaseUrls[providerName], sig), cfg.providerAttemptTimeoutMs, signal);
                    if (entry)
                        markUsed(entries, index);
                    return {
                        url: request.url,
