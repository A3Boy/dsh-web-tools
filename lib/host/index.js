...
export function apply(ctx) {
    ...
    const routedSearchProvider = {
        ...
        search: (request, signal) => defaultSourceRegistry.routeSearch(request, generalSearchProvider, signal),
    };
    ...
    const routedFetchProvider = {
        ...
        fetch: (request, signal) => defaultSourceRegistry.routeFetch(request.url, generalFetchProvider, signal),
    };
    ...
    defaultBridgeServer.setPersistHook((hash) => {
    ...
    if (typeof ctx.webServer.registerUpgrade === "function") {
        ctx.effect(() => defaultBridgeServer.registerUpgradeRoute(ctx.webServer), "dsh-web-tools: browser-bridge websocket");
    }
