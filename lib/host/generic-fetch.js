 ...
 *  - SSRF URL and redirect validation via `fetch-security.ts`
 ...
 *  - Manual redirect following (max 5 hops) with per-hop SSRF validation
 *  - Markdown conversion via Defuddle + linkedom for HTML content
...
import { validateFetchUrl, FetchSecurityError } from "./fetch-security.js";
 ...
 */
export async function fetchGenericWebPage(initialUrl, signal, customFetchWithProxy = fetchWithProxy) {
    let currentUrl = validateFetchUrl(initialUrl).href;
    let hopCount = 0;
    let response;
    // Manual redirect loop to validate SSRF on every hop
    while (true) {
        if (signal?.aborted) {
            throw new GenericFetchError("WEB_ABORTED", "Fetch aborted by signal");
        }
        ...
        try {
            response = await customFetchWithProxy(currentUrl, {
                method: "GET",
                headers: {
        ...
        if (response.status === 301 ||
            ...
            // Re-validate target URL for SSRF / loopback / private IP
            try {
    ...
    const { text: bodyText, truncated } = await readStreamBounded(response, MAX_FETCH_BYTES, signal);
    ...
    // HTML content: Parse with linkedom + Defuddle
    try {
