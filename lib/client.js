	...
	factory: (require) => {
		...
		*/
		...
		function quotaMetaLine(t, q) {
			...
			if (kind === "observed_usage" && q.used !== void 0) return t("quotaSince", { amount: (q.used / 100).toFixed(2) });
			return "";
		}
		...
		*/
		...
		for (const [name, c] of Object.entries(COLORS)) PROVIDER_BRAND[name] = {
			icon: svgDataUri(makeSvg(c.bg, c.letter)),
			label: c.label
		};
		//#endregion
