	...
	factory: (require) => {
		...
		*/
		function QuotaBar(props) {
			...
			const tierColor = tier === "danger" ? state.danger : tier === "warn" ? state.warning : accent.primary;
		...
		function ProviderModal(props) {
			...
			const status = (testResult !== void 0 && !testResult.ok ? testOutcomeStatus(testResult) : void 0) ?? providerStatusOf(p, quota, inChain);
			const statusText = {
				ready: t("ready"),
		...
		*  chain render non-draggable with a "not in chain" tag. */
		function ProviderCard(props) {
			...
			const inChain = orderRank !== void 0;
		...
		function ProviderQuotaInline(props) {
			...
			const tier = quotaTier(fraction);
			const tierColor = tier === "danger" ? state.danger : tier === "warn" ? state.warning : accentText();
      ...
      ` }),
		...
		*/
		function registerSettingsSection(ctx, t, component) {
		...
		*/
		...
		function apply(ctx) {
			...
			registerSettingsSection(ctx, ctx.locale.bind(NS), WebToolsSection);
		}
		//#endregion
		exports.NS = NS;
		exports.apply = apply;
